            ...
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <h5 class="card-title mb-0">
                    <div class="row">
                        ...
                        @php
                            $columnClass = isset($subjects) ? 'col-7' : 'col-12';
                        @endphp
                            ...
                            <tbody>
                                @foreach ($subjects as $index => $subject)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($subject->staff)
                                                <a href="{{ route('admin.staffs.edit', $subject->staff_id) }}">
                                                    {{ $subject->staff->fullname }}</a>
                                            @else
                                                <span class="text-danger">N/A</span>
                                            @endif
                                        </td>
                                        <td>
                                            {{ $subject->level->short_name ?? 'N/A' }}
                                        </td>
                            ...
                            <tbody>
                                @foreach ($levels as $index => $level)
                                    <tr>
                                        ...
                                        <td><a href="{{ route('admin.subjects.level', $level->id) }}"
                                        ...
                                        <td class="d-flex gap-2 align-items-center">
                                            {{-- Edit Button --}}
                                            <a href="{{ route('admin.subjects.level', $level->id) }}"
                                                class="btn btn-outline-primary-600 radius-8 text-primary-600 d-inline-flex align-items-center justify-content-center gap-1">
                                                <iconify-icon icon="icon-park-outline:to-left"
                                                    class="text-xl"></iconify-icon>
                                                <span>Subjects [{{ count($level->subjects) }}]</span>
                                            </a>
